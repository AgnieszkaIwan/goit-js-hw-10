...
searchEl.addEventListener("input", (0, _lodashDebounceDefault.default)(async (ev)=>{
    ...
    try {
        const countries = await (0, _fetchCountriesJs.fetchCountries)(countryName);
        if (countries.length > 10) (0, _notiflix.Notify).info("Too many matches found. Please enter a more specific name.");
        else countryListEl.innerHTML = countries.map((country)=>`<li><img src="${country.flags.png}"/> ${country.name.common}</li>`).join("");
        if (countries.length === 1) countryInfo.innerHTML = `
        ...
        if (error.message === "Country not found") (0, _notiflix.Notify).failure("Oops, there is no country with that name");
        else (0, _notiflix.Notify).failure("Something went wrong. Please try again later.");
    }
}, DEBOUNCE_DELAY));
...
const fetchCountries = (name)=>{
    return fetch(`${API_URL}/name/${name}?fields=name,capital,population,flags,languages`).then((res)=>{
